...
import CpProfilePair from './CpProfilePair';
...
function ProfileCardContent({ userId, onClose, socket }) {
  ...
  const [nestedProfileId, setNestedProfileId] = useState(null);
  ...
  return (
    <>
      {nestedProfileId && nestedProfileId !== userId && (
        <ProfileCardContent
      ...
      >
        ...
        >
          ...
          {loading ? (
            <div style={{ padding: 40, textAlign: 'center', color: '#9ca3af' }}>Yukleniyor...</div>
          ) : profile ? (
            <>
              <div style={{ padding: '20px 56px 16px 20px' }}>
                ...
                <div style={{ marginTop: 8, fontSize: 11, color: '#4b5563' }}>
                  Uyelik: {new Date(profile.createdAt).toLocaleDateString('tr-TR')}
                </div>
              </div>
              ...
              <div style={{ padding: '0 16px 12px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <CpProfilePair
                  ...
                  profileUser={profile}
                  socket={socket}
                  onOpenProfile={(pid) => setNestedProfileId(pid)}
                />
              </div>
              ...
              {me && me.id !== userId && (
                <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {friendStatus === 'friends' ? (
                    ...
                    >
                      {friendStatus === 'pending' ? '⌛ Istek Gonderildi' : '+ Arkadas Ekle'}
                    </button>
                  )}
                  <button
                    type="button"
                  ...
                  {sentMsg && <div style={{ fontSize: 12, color: '#86efac', textAlign: 'center' }}>✓ Mesaj gonderildi</div>}
                </div>
              )}
            </>
          ) : (
            <div style={{ padding: 40, textAlign: 'center', color: '#9ca3af' }}>Kullanici bulunamadi</div>
          )}
        </div>
      </div>
    </>
